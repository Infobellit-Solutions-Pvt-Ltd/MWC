...
def send_data():
    ...
    #while True:  # The loop will run indefinitely unless manually stopped
        ...
        file = "/home/arunGPU12/workspace/Arun/automation/Sarthak/EchoSwift/test_results/testing_run5/1_User/avg_32_input_tokens.csv"
        ...
        #data = df.to_json('benchmark_results.json', index=False, indent=4)
        #count += 1
        publish('1P_LLM', json_data)  # Using JSON for better structure in the message
...
connection_obj, channel_obj = None, None


def get_connection():
    global connection_obj, channel_obj

    if connection_obj is None or connection_obj.is_closed:
...
def send_message(channel, msg, queue_name):
...
def publish_message(queue, message):
    ...
    send_message(channel, message, queue)


#publish_message('dummy', json.dumps({'Queue_Name': 'dummy'}))
