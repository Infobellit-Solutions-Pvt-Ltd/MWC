...
echo "" >> "$LOG_FILE"
echo "==== Starting LLM containers on 2PC Server ====" >> "$LOG_FILE"
echo "Script started at: $(date)" >> "$LOG_FILE"

echo "Generating the docker-compose.yml and haproxy.cfg files for Deepseek" >> "$LLM_LOG_FILE" 2>&1
ssh -q -o "LogLevel=ERROR" -t 2pc_a << 'EOF' >> "$LLM_LOG_FILE" 2>&1
  ...
  REPLICAS=$DOCKER_COMP_NUM_CONTAINERS_2PC_LLM
...
echo "Files generated successfully. Starting the Deepseek containers on 2PC server." >> "$LLM_LOG_FILE" 2>&1
ssh -q -o "LogLevel=ERROR" -t 2pc_a "cd ./demo/docker_compose/DS && docker-compose up -d"  >> "$LLM_LOG_FILE" 2>&1
...
EOF
echo "===== Deployment Script Completed on 2PC Server ====" >> "$LOG_FILE"