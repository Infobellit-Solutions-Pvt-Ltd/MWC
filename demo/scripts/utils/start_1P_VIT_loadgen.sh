...
CONFIG_FILE=1P_VIT_config.json
...
try:
    ...
    config["topic"] = "$VIT_1P_TOPIC"
    config["mqtt_user"] = "$RABBITMQ_USER"
    config["mqtt_pass"] = "$RABBITMQ_PASSWORD"
    config["mqtt_ip"] = "$RABBITMQ_HOST"
    config["base_url"] = "$ENDPOINT_1P_VIT"
    config["max_requests"] = $VIT_1P_SAMPLE_SIZE
    config["user_counts"] = [$DOCKER_COMP_NUM_CONTAINERS_1P_VIT]
...
docker run -d --rm --name "1P_loadgen_vit" -e CONFIG_PATH=/app/EchoSwift/config.json \
           -v ./1P_VIT_config.json:/app/EchoSwift/config.json \
