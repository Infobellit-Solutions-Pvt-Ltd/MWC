...
CONFIG_FILE=2P_VIT_config.json
...
try:
    ...
    config["topic"] = "$VIT_2P_TOPIC"
    config["mqtt_user"] = "$RABBITMQ_USER"
    config["mqtt_pass"] = "$RABBITMQ_PASSWORD"
    config["mqtt_ip"] = "$RABBITMQ_HOST"
    config["base_url"] = "$ENDPOINT_2P_VIT"
    config["max_requests"] = $VIT_2P_SAMPLE_SIZE
    config["user_counts"] = [$DOCKER_COMP_NUM_CONTAINERS_2P_VIT]
...
docker run -d --rm --name "2P_loadgen_vit" -e CONFIG_PATH=/app/EchoSwift/config.json \
           -v ./2P_VIT_config.json:/app/EchoSwift/config.json \
