...
do
  ...
  curl -s -X POST "${ENDPOINT_2P_LLM_DS}" \
...
docker run -d --rm --name "2P_loadgen_DS" -e CONFIG_PATH=/app/EchoSwift/config.json \
           -v ./2P_DS_config.json:/app/EchoSwift/config.json \
