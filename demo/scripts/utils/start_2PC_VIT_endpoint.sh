...
echo "" >> "$LOG_FILE"
echo "==== Starting VIT Service on 2PC Server ====" >> "$LOG_FILE"
echo "Script started at: $(date)" >> "$LOG_FILE"

echo "Generating the docker-compose.yml and haproxy.cfg files for VIT" >> "$VIT_LOG_FILE" 2>&1
ssh -q -o "LogLevel=ERROR" -t 2pc_b << 'EOF' >> "$VIT_LOG_FILE" 2>&1
  ...
  REPLICAS=$DOCKER_COMP_NUM_CONTAINERS_2PC_VIT
...
echo "Files generated successfully. Starting the VIT containers on 2PC server." >> "$VIT_LOG_FILE" 2>&1
ssh -q -o "LogLevel=ERROR" -t 2pc_b "cd ./demo/docker_compose/VIT && docker-compose up -d"  >> "$VIT_LOG_FILE" 2>&1
