...
CONFIG_FILE="2PC_DS_config.json"
...
do
  ...
  curl -s -X POST "${ENDPOINT_2PC_LLM_DS}" \
...
docker run -d --rm --name "2PC_loadgen_DS" -e CONFIG_PATH=/app/EchoSwift/config.json \
           -v ./2PC_DS_config.json:/app/EchoSwift/config.json \
